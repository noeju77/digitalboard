...
if (isset($_POST['carpeta']) && isset($_POST['tituloContenido'])) {
    ...
    if ($_FILES['fichero']['error'] <= 0 && array_search($_FILES['fichero']['type'], $tiposAceptados) && $_FILES['fichero']['size'] <= 1024 * 20 * 20) {
        move_uploaded_file($_FILES['fichero']['tmp_name'], "../res/" . $_FILES['fichero']['name']);
    }

    echo $daoResources->insertar($resource1);
}
